...
class DistanceRankFusion:
    def __init__(self, df: pd.DataFrame, score_columns: List[str], algorithm: str = 'rrf', weights: Optional[List[float]] = None, k: int = 60):
        """
        ...
        if algorithm == 'rrf':
            self.result_df = self._reciprocal_rank_fusion(k)
        elif algorithm == 'isr':
            self.result_df = self._inverse_square_rank_fusion(k)
        elif algorithm == 'rdf':
    ...
    def _reciprocal_rank_fusion(self, k: int) -> pd.DataFrame:
        ...
        for col in self.score_columns:
            ...
            df_copy[rank_col] = df_copy[col].rank(method='min', ascending=True)  # min value is rank 1
            df_copy[col] = 1 / (df_copy[rank_col] + k)
        df_copy['RRF_score'] = df_copy[self.score_columns].sum(axis=1)
        return df_copy.sort_values('RRF_score', ascending=False)
    ...
    def _inverse_square_rank_fusion(self, k: int) -> pd.DataFrame:
        ...
        for col in self.score_columns:
            rank_col = f"{col}_rank"
            df_copy[rank_col] = df_copy[col].rank(method='min', ascending=True)  # min value is rank 1
            df_copy[col] = 1 / ((df_copy[rank_col] + k) ** 2)
        df_copy['ISR_score'] = df_copy[self.score_columns].sum(axis=1)
        return df_copy.sort_values('ISR_score', ascending=False)
    ...
    def _relative_distance_fusion_min_max(self, weights: Optional[List[float]]) -> pd.DataFrame:
        ...
        for col in self.score_columns:
            ...
            df_copy[normalized_col] = (max_score - df_copy[col]) / (max_score - min_score)  # reverse the scores
        for col, weight in zip(self.score_columns, weights):
            normalized_col = f"{col}_normalized"
            df_copy[normalized_col] *= weight
        df_copy['RDF_MinMax_score'] = df_copy[[f"{col}_normalized" for col in self.score_columns]].sum(axis=1)
        return df_copy.sort_values('RDF_MinMax_score', ascending=False)  # lower score is better
    ...
    def _relative_distance_fusion_dist_based(self, weights: Optional[List[float]]) -> pd.DataFrame:
        ...
        for col in self.score_columns:
            ...
            df_copy[normalized_col] = (max_score - df_copy[col]) / (max_score - min_score)
        for col, weight in zip(self.score_columns, weights):
            normalized_col = f"{col}_normalized"
            df_copy[normalized_col] *= weight
        df_copy['RDF_DistBased_score'] = df_copy[[f"{col}_normalized" for col in self.score_columns]].sum(axis=1)
        return df_copy.sort_values('RDF_DistBased_score', ascending=False) # lower score is better
    ...
    def get_result(self, show_details: bool = False) -> pd.DataFrame:
        ...
        """
        if not show_details:
            columns_to_drop = [col for col in self.result_df.columns if '_rank' in col or '_normalized' in col]
            self.result_df = self.result_df.drop(columns=columns_to_drop)
